...
namespace NodeGraph.App.Converters;
...
public class TypeToColorConverter : IValueConverter
{
    ...
    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        if (PrimitiveTypeColors.TryGetValue(typeName, out var color)) return new SolidColorBrush(color);
    ...
    {
        var hi = System.Convert.ToInt32(Math.Floor(hue / 60)) % 6;
        var f = hue / 60 - Math.Floor(hue / 60);

        value = value * 255;
        var v = System.Convert.ToInt32(value);
        var p = System.Convert.ToInt32(value * (1 - saturation));
        var q = System.Convert.ToInt32(value * (1 - f * saturation));
        var t = System.Convert.ToInt32(value * (1 - (1 - f) * saturation));
