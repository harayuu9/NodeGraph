    ...
    public class TopologicalLayoutService : IGraphLayoutService
    {
            ...
            Func<EditorNode, (double width, double height)> getNodeSize)
        {
            ...
            for (var i = 1; i < layers.Count; ++i)
                OrderLayerByBarycenter(layers[i], layers[i - 1], edges, previousLayer:true);
            ...
            for (var i = 1; i < layers.Count; ++i)
            {
                ...
                foreach (var v in layers[i])
                {
                    ...
                    {
                        x = Math.Max(x, p.X + nodeSize[p].width + minHorizontal);
                    }
                }
                ...
                double bcGapPx)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    int inCnt  = inMap.TryGetValue(u, out var inE)  ? inE.Count  : 0;
                    int outCnt = outMap.TryGetValue(u, out var outE) ? outE.Count : 0;
                    degs[i] = inCnt + outCnt;
                    ...
                    if (outMap.TryGetValue(u, out var outEdges))
                    {
                        foreach (var e in outEdges)
                        {
                            ...
                            int inCount = GetInputPortCount(v);
                            inCount = Math.Max(inCount, 1);
                            int pidx = GetTargetPortIndex(e);
                            pidx = Math.Clamp(pidx, 0, inCount - 1);
                            double gap = vh / (inCount + 1);
                            double anchor = v.Y + gap * (pidx + 1);     // v の入力ポートアンカーY
                            anchors.Add(anchor);
                        }
                    }
                ...
                {
                    double degAdd = degGapPx * 0.5 * (degs[i] + degs[i + 1]);     // 片側平均
                    double bcAdd  = (!double.IsNaN(bc[i]) && !double.IsNaN(bc[i + 1]))
                                    ? bcGapPx * Math.Abs(bc[i + 1] - bc[i])
                                    : 0.0;
                    // 必要なら上限を掛けて暴れを抑制
                    double extra = degAdd + bcAdd;
                ...
                for (int i = 1; i < n; i++) offset[i] = offset[i - 1] + delta[i - 1];
                var tStar = new double[n];
                for (int i = 0; i < n; i++) tStar[i] = centersStar[i] - offset[i];
                ...
                for (int i = 0; i < n; i++)
                {
                    double center = tStar[i] + offset[i];
                ...
                List<EditorConnection> edges)
            {
                ...
                foreach (var u in prev)
                {
                    ...
                    score[u] = outs.Count == 0 ? double.PositiveInfinity
                ...
                while (ready.Count > 0)
                {
                    ...
                    var u = ready[0]; ready.RemoveAt(0);
                    order.Add(u);
            ...
            static int GetTargetPortIndex(EditorConnection connection) => connection.TargetNode.InputPorts.IndexOf(connection.TargetPort);

            static int GetInputPortCount(EditorNode editorNode) => editorNode.InputPorts.Count;
        }
    }
}
